...
class UNet_Adv:
    ...
    def Conv2D_block_1(self,input_tensor, kernel_size, filters, activation = "relu"):
        ...
        conv = BatchNormalization(renorm=True)(conv)
         ...
         #----------- Residual Block --------------
        conv_r = Conv2D(filters=filters, kernel_size=(1,1),
                        kernel_initializer="he_normal", padding="same")(input_tensor)
        conv_r = BatchNormalization(renorm=True)(conv_r)
        conv = tf.keras.layers.Add()([conv, conv_r])
        conv = Activation(activation)(conv)
        conv = BatchNormalization(renorm=True)(conv)
        #-----------------------------------------
    ...
    def Conv2D_block_Down(self,input_tensor, kernel_size, filters, activation = "relu"):
        conv1 = Conv2D(filters=filters, kernel_size=kernel_size,
                    kernel_initializer="he_normal", padding="same")(input_tensor)
        conv1 = Activation(activation)(conv1)           
        conv1 = BatchNormalization(renorm=True)(conv1)
        ...
        #----------- Residual Block --------------
        conv_r = tf.keras.layers.Add()([conv, conv1])
        conv_r = Activation(activation)(conv_r)
        conv_r = BatchNormalization(renorm=True)(conv_r)
        #-----------------------------------------

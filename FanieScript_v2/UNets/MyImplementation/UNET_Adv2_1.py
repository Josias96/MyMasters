...
class UNet_Adv:
    ...
    def Conv2D_block_1(self,input_tensor, kernel_size, filters):
            ...
            #----------- Residual Block --------------
            conv_in = DepthwiseConv2D(depth_multiplier=2, kernel_size=kernel_size,
    ...
    def CreateUnet(self):
        ...
        c6 = self.Conv2D_block_Up(u1, kernel_size=(3, 3), filters=self.n_filters*8)

        u2 = self.UpConvolution(c6, c3, kernel_size=(2, 2), filters=self.n_filters*4)
        c7 = self.Conv2D_block_Up(u2, kernel_size=(3, 3), filters=self.n_filters*4)

        u3 = self.UpConvolution(c7, c2, kernel_size=(2, 2), filters=self.n_filters*2)
        c8 = self.Conv2D_block_Up(u3, kernel_size=(3, 3), filters=self.n_filters*2)

        u4 = self.UpConvolution(c8, c1, kernel_size=(2, 2), filters=self.n_filters)
        c9 = self.Conv2D_block_Up_Last(u4, kernel_size=(3, 3), filters=self.n_filters)

        output_layer = Conv2D(filters=1, kernel_size=(1, 1),
                            activation="sigmoid", name="Convolution_c9")(c9)
